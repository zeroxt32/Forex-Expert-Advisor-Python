...
# print(f"current_trade {current_trade}\n")
    ...
    def expert_choices_null(self, index):
        choices = list(range(12, 23,1))
        choice2 = list(range(25, 38 , 1))
        choice3 = list(range(5,11,1))
        choice4 = list(range(39,45, 1))
    ...
    def train_expert_advice(self):
        ...
        for episode in range(4,550,1):
            ...
            if not done:
                ...
                target_f = reward + self.gamma * ( 1 - done ) * target_f_values[0][0]
            else:
                target_f = reward
            q_values = self.actor.predict(state)
            try:
            ...
            self.critic.fit(state, np.array(reward + self.gamma * (1-done) * target_f_values ).reshape(1,1,1) , verbose=2)
...
for _ in range(100):
    ...
    #if _ < 1:
    #    agent.epsilon = 1
    #    agent.train(1, False, agent.epsilon)
    #if _ >= 3:
    #agent.epsilon = 0.001
    #    agent.train(2, False, agent.epsilon)
    ...
    #clear_output(wait=True)    
    agent.train(20, False, agent.epsilon)
